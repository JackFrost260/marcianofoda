...
public class RamSplineEditor : Editor
{
    ...
    private void ShowDebugHandles()
    {
        ...
        for (int i = 0; i < spline.pointsDown.Count; i++)
        {
            Handles.SphereHandleCap(0, spline.pointsDown[i] + spline.transform.position, Quaternion.identity, 0.2f, EventType.Repaint);
        ...
        {
            Handles.SphereHandleCap(0, spline.pointsUp[i] + spline.transform.position, Quaternion.identity, 0.2f, EventType.Repaint);
        ...
        else if (meshDebug)
        {
            ...
            if (mesh)
            {
                ...
                for (int i = 0; i < vertices.Length; i++)
                {
                    ...
                    Handles.color = Color.magenta;
                    Handles.DrawLine(vertices[i], vertices[i] + new Vector3(uv4[i].x, uv4[i].y, 0) * 10);
                }
